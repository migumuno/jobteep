...
<div class = "medium-10 medium-offset-1 columns">
	<div class = "row">
		<ul class="breadcrumbs">
		  ...
		  <li><a href="?program=panel&menu=mtlculture">Cultura</a></li>
...
<div class = "row">
	<div class = "small-10 small-offset-1 columns panel">
		<form data-abide method = "post" action = "?program=panel&menu=<?php echo $enum ?>&action=<?php echo $action ?>" enctype="multipart/form-data">
			...
			<div class = "row">
				<div class = "medium-4 columns">
					<label>Elige la categoría <small>Requerida</small>
						<input type = "text" list = "categories" name = "category" placeholder="Cine / Lectura / Teatro ..." value = "<?php echo $controller->getField ($MEMO, 'category') ?>" required autocomplete = "off"/>
					</label>
					<datalist id = "categories">
			        	<?php 
			        	$controller = $_SESSION['SO']->getController();
			        	$result = $controller->getData('culture');
			...
			<div class = "row">
				<div class = "medium-4 columns">
					<label>¿Cual es el genero? <small>Requerido</small>
						<input type = "text" list = "genres" name = "genre" placeholder="Histórico / Biográfico / Comedia ..." value = "<?php echo $controller->getField ($MEMO, 'genre') ?>" required autocomplete = "off"/>
					</label>
					<datalist id = "genres">
			        	<?php 
			        	$controller = $_SESSION['SO']->getController();
			        	$result = $controller->getData('genre');
			        	for($i = 0; $i < $result->num_rows; $i++) {
							$result->data_seek($i);
							$row = $result->fetch_assoc();
							echo '<option value = "'.$row['name'].'">';
			        ...
			        <small class="error">Necesitamos saber el genero.</small>
				</div>
			</div>
			<div class="row">
			    <div class="medium-12 columns">
			      <label>Crítica <small>Importante</small>
		  ...
		  <fieldset>
		  <legend>Puedes Añadir una Imagen de la Obra</legend>
			<div class = "row">
			  	<div class = "medium-12 columns">
			  		<?php 
